...
/// Describes how audio samples are packaged inside an HLS stream.
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
...
impl FromStr for HlsSegmentFormat {
    type Err = Error;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "AAC" | "aac" => Ok(Self::Aac),
            "AC3" | "ac3" => Ok(Self::Ac3),
            "E_AC3" | "e-ac3" => Ok(Self::EAc3),
            "FMP4" | "fmp4" => Ok(Self::Fmp4),
            "MP3" | "mp3" => Ok(Self::Mp3),
            "TS" | "ts" => Ok(Self::Ts),
            "TS_AAC" | "ts_aac" => Ok(Self::TsAac),
            _ => Err(Error::Parsing(format!(
                "Unknown HLS audio segment format {s}"
            ))),
        }
    }
}

impl fmt::Display for HlsSegmentFormat {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
/// Describes how video samples are packaged inside an HLS stream.
///
/// Cast assumes MPEG-2 transport stream segments when this value is omitted,
/// so senders must explicitly select [`HlsVideoSegmentFormat::Fmp4`] for CMAF
/// or fragmented MP4 playlists.
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum HlsVideoSegmentFormat {
    /// Video packed in ISO BMFF CMAF fragmented MP4 segments.
    Fmp4,
    /// Video packed in MPEG-2 transport stream segments.
    Mpeg2Ts,
}

impl FromStr for HlsVideoSegmentFormat {
    type Err = Error;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "FMP4" | "fmp4" => Ok(Self::Fmp4),
            "MPEG2_TS" | "mpeg2_ts" => Ok(Self::Mpeg2Ts),
            _ => Err(Error::Parsing(format!(
                "Unknown HLS video segment format {s}"
            ))),
        }
    }
}

impl fmt::Display for HlsVideoSegmentFormat {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let format = match self {
            Self::Fmp4 => "fmp4",
            Self::Mpeg2Ts => "mpeg2_ts",
        };
        write!(f, "{format}")
    }
}
...
pub struct Media {
    ...
    /// Packaging format of audio segments in an HLS playlist.
    pub hls_segment_format: Option<HlsSegmentFormat>,
    /// Packaging format of video segments in an HLS playlist.
...
impl Media {
    fn encode(&self) -> proxies::media::Media {
        ...
        proxies::media::Media {
            ...
            hls_segment_format: self.hls_segment_format.map(|value| value.to_string()),
            hls_video_segment_format: self.hls_video_segment_format.map(|value| value.to_string()),
            metadata,
...
impl TryFrom<&proxies::media::Media> for Media {
    ...
    fn try_from(m: &proxies::media::Media) -> Result<Self, Error> {
        Ok(Self {
            ...
            hls_segment_format: m
                .hls_segment_format
                .as_deref()
                .map(HlsSegmentFormat::from_str)
                .transpose()?,
            hls_video_segment_format: m
                .hls_video_segment_format
                .as_deref()
                .map(HlsVideoSegmentFormat::from_str)
                .transpose()?,
...
pub struct LoadOptions {
    /// The current time of the content to start the playback at. Leave this
    /// unset to let a live stream start at its live position.
...
mod tests {
    ...
    #[test]
    fn test_encode_fmp4_hls_media() {
        let media = Media {
            content_id: "http://example.test/live.m3u8".to_owned(),
        ...
        let encoded = serde_json::to_value(media.encode()).unwrap();

        assert_eq!(encoded["hlsSegmentFormat"], "fmp4");
        assert_eq!(encoded["hlsVideoSegmentFormat"], "fmp4");
        assert_eq!(encoded["duration"], -1.0);
    }

    #[test]
    fn test_live_load_omits_current_time_for_live_edge() {
        let media = Media {
            content_id: "http://example.test/live.m3u8".to_owned(),
            stream_type: StreamType::Live,
            content_type: "application/x-mpegURL".to_owned(),
        ...
        let request = proxies::media::MediaRequest {
            request_id: 1,
            session_id: "session".to_owned(),
            typ: MESSAGE_TYPE_LOAD.to_owned(),
            media: media.encode(),
            current_time: LoadOptions::default().current_time,
            autoplay: true,
            custom_data: proxies::media::CustomData::new(),
            queue_data: None,
        };
        ...
        assert!(encoded.get("currentTime").is_none());
    }
