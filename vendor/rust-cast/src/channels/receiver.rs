    ...
    W: Write + Read,
{
    ...
    pub fn broadcast_message<M: Serialize>(
        ...
        self.send_message("*", namespace, message)
    }

    /// Sends a JSON message to a specific Cast application transport.
    ///
    /// This is used by application protocols such as Cast Streaming, where
    /// messages must be addressed to the `transport_id` returned by
    /// [`launch_app`](Self::launch_app), rather than broadcast to every app.
    pub fn send_message<M: Serialize>(
        &self,
        destination: &str,
        namespace: &str,
        message: &M,
